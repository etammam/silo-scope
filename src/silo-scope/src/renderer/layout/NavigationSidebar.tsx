...
import type { NugetFeed } from "../../features/feeds/schema";
...
const LARGE_CATALOG_FUNCTION_THRESHOLD = 40;
const LARGE_CATALOG_INTERFACE_THRESHOLD = 18;
...
function WorkspaceNavigator({
  ...
  const isLargeCatalog =
    catalogMetrics.functions > LARGE_CATALOG_FUNCTION_THRESHOLD ||
    catalogMetrics.interfaces > LARGE_CATALOG_INTERFACE_THRESHOLD;
  ...
  const showMoreInterfaces = (sourceId: string) => {
    setVisibleInterfaceLimits((current) => {
      const next = new Map(current);
      next.set(sourceId, (next.get(sourceId) ?? INTERFACE_PREVIEW_LIMIT) + INTERFACE_PREVIEW_LIMIT);
      return next;
    });
  };
                ...
                {source.interfaces.length > 0 && !collapsedSources.has(source.sourceId) && (
                  <div className="navigation-sidebar__source-children">
                    <SourceInterfaceTree
                      ...
                      selectedGrain={selectedGrain}
                      source={source}
                      visibleInterfaceLimit={visibleInterfaceLimits.get(source.sourceId) ?? INTERFACE_PREVIEW_LIMIT}
                    />
                  </div>
                )}
              </li>
    ...
    <ul className="navigation-sidebar__tree navigation-sidebar__tree--nested">
      {visibleInterfaces.map((catalogInterface) => {
        ...
        const visibleMethods = fullyVisibleInterfaces.has(key)
          ...
          : catalogInterface.methods.slice(0, METHOD_PREVIEW_LIMIT);
      ...
      {hiddenInterfaceCount > 0 && (
        <li className="navigation-sidebar__load-more-item">
          <button
            ...
            type="button"
          >
            Show next {Math.min(hiddenInterfaceCount, INTERFACE_PREVIEW_LIMIT).toLocaleString()} of{" "}
...
export function NuGetRegistryManager({
  ...
  const availableFeeds = useMemo(
    () => [
      DEFAULT_NUGET_FEED,
      ...feeds.filter((feed) => feed.name !== DEFAULT_NUGET_FEED.name),
    ],
    [feeds],
  );
        ...
        <section className="feed-manager__panel" aria-labelledby="configured-feeds-title">
          ...
          <ul className="feed-manager__feeds" aria-label="Configured feeds">
            <li className="feed-manager__feed-card" data-default="true">
              <div>
                <strong>{DEFAULT_NUGET_FEED.name}</strong>
                <span>{DEFAULT_NUGET_FEED.url}</span>
              </div>
              <small>Default</small>
            </li>
