...
import type { NugetFeed, NugetPackage } from "../../shared/types";
...
const emptyForm: FeedFormState = { name: "", url: "", username: "", password: "" };
...
export function PackageFeedsPage({
  ...
  const canSubmitForm = form.name.trim().length > 0 && form.url.trim().length > 0;
  ...
  const handleSaveFeed = useCallback(async () => {
    ...
    try {
      ...
      setTestState("error");
      setTestMessage(error instanceof Error ? error.message : "Failed to save feed");
    } finally {
      setFeedSaveState("idle");
    }
  ...
  const performSearch = useCallback(
    async (searchQuery: string) => {
      ...
      try {
        const pkgs = await onSearchPackages(searchQuery.trim(), selectedFeedName ?? undefined, 10);
        setResults(pkgs);
      } catch (error) {
  ...
  const togglePackage = useCallback(
    async (packageId: string) => {
      ...
      try {
        if (onGetVersions) {
          const versions = await onGetVersions(packageId, selectedFeedName ?? undefined);
  ...
  const handleSearchKeyDown = useCallback(
    (event: ReactKeyboardEvent<HTMLInputElement>) => {
      ...
      if (event.key === "Enter" && focusedResultIndex >= 0 && focusedResultIndex < results.length) {
        event.preventDefault();
              ...
              <div className="pkg-feeds__search-panel">
                <div className="pkg-feeds__search-row">
                  <label className="pkg-feeds__search">
                    ...
                    {isSearching ? (
                      <Loader2 aria-hidden="true" className="pkg-feeds__search-icon pkg-feeds__search-icon--spinning" width={16} height={16} />
                    ) : (
                      <Search aria-hidden="true" className="pkg-feeds__search-icon" width={16} height={16} />
                    )}
                    <input
                      ref={searchInputRef}
                      ...
                      aria-activedescendant={focusedResultIndex >= 0 ? `pkg-result-${results[focusedResultIndex]?.packageId}` : undefined}
                    />
                    {query.length > 0 && (
                      <button aria-label="Clear search" className="pkg-feeds__search-clear" onClick={clearSearch} type="button">
                        <X aria-hidden="true" width={15} height={15} />
                      </button>
                    )}
                    <kbd className="pkg-feeds__search-kbd" aria-hidden="true">/</kbd>
                  </label>
                ...
                {searchError && (
                  <div className="pkg-feeds__browser-error">
                    ...
                    <button className="pkg-feeds__btn pkg-feeds__btn--secondary pkg-feeds__btn--small" onClick={() => performSearch(query)} type="button">Retry</button>
                  </div>
                )}
                ...
                {results.length > 0 && (
                  <>
                    <div className="pkg-feeds__results-summary">
                      ...
                      {selectedFeedName ? ` in ${selectedFeedName}` : " across all feeds"}
                      {query.trim() && <> matching &ldquo;{query.trim()}&rdquo;</>}
                    </div>
                    <div id="pkg-results-list" className="pkg-feeds__results" role="listbox" aria-label="Package search results">
                      {results.map((pkg, index) => {
                        ...
                        return (
                          ...
                          >
                            <button className="pkg-feeds__result-summary" onClick={() => togglePackage(pkg.packageId)} type="button">
                              <span className="pkg-feeds__result-icon"><Package aria-hidden="true" width={18} height={18} /></span>
                              <span className="pkg-feeds__result-main">
                                <span className="pkg-feeds__result-id">{pkg.packageId}</span>
                                <span className="pkg-feeds__result-meta">
                                  <span className="pkg-feeds__result-version">v{pkg.version}</span>
                                  {pkg.authors && <span className="pkg-feeds__result-author">by {pkg.authors}</span>}
                                  {dl && <span className="pkg-feeds__result-downloads"><Download aria-hidden="true" width={11} height={11} />{dl}</span>}
                                </span>
                              </span>
                              <span className="pkg-feeds__result-chevron" aria-hidden="true"><ChevronRight width={16} height={16} /></span>
                            </button>
                            {pkg.description && !isExpanded && <p className="pkg-feeds__result-desc">{pkg.description}</p>}
                            {isExpanded && (
                              <div className="pkg-feeds__result-detail">
                                {pkg.description && <p className="pkg-feeds__result-detail-desc">{pkg.description}</p>}
                                <dl className="pkg-feeds__result-meta-list">
                                  {pkg.authors && <><dt>Authors</dt><dd>{pkg.authors}</dd></>}
                                  {dl && <><dt>Downloads</dt><dd>{dl}</dd></>}
                                  <dt>Source</dt>
                                  <dd>{selectedFeedName ?? feeds.find((f) => f.isDefault)?.name ?? "Unknown"}</dd>
                                </dl>
                                <div className="pkg-feeds__versions">
                                  <span className="pkg-feeds__versions-label">Version</span>
                                  {isLoadingVersions ? (
                                    <div className="pkg-feeds__versions-loading"><Loader2 aria-hidden="true" width={14} height={14} /><span>Loading versions…</span></div>
                                  ) : packageVersions.length > 0 ? (
                                    <div className="pkg-feeds__versions-list" role="radiogroup" aria-label="Package versions">
                                      {packageVersions.map((v) => (
                                        <button key={v} className={`pkg-feeds__version-chip${selectedVersion === v ? " pkg-feeds__version-chip--selected" : ""}`} role="radio" aria-checked={selectedVersion === v} onClick={() => setSelectedVersion(v)} type="button">{v}</button>
                                      ))}
                                    </div>
                                  ) : (
                                    <span className="pkg-feeds__versions-none">No version information available</span>
                                  )}
                                </div>
                              </div>
                            )}
                ...
                {!isSearching && !searchError && query.trim() && results.length === 0 && (
                  <div className="pkg-feeds__search-empty">
                    ...
                    <span>No packages found for &ldquo;{query.trim()}&rdquo;</span>
                  </div>
                )}
              </div>
            </div>
